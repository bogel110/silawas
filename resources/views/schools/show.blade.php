                ...
                <div class="d-flex align-items-center gap-3">
                    ...
                    @if(auth()->user()->role === 'admin_sekolah' && auth()->user()->school_id == $school->id)
                        <button type="button" class="btn btn-sm btn-outline-primary fw-bold d-flex align-items-center gap-1 mt-2" data-bs-toggle="modal" data-bs-target="#modalDokumenMaster">
                            <span class="material-symbols-outlined fs-6">edit</span> Update Dokumen
                        </button>
                    @endif
                </div>
            </div>
        ...
        <div class="col-lg-4">
            <div class="card border-0 shadow-sm rounded-4 mb-4">
                <div class="card-header bg-white border-bottom-0 pt-4 pb-0 px-4">
                    <h5 class="font-headline fw-bold mb-0">Modul 1: Administrasi</h5>
                </div>
                <div class="card-body p-4">
                    <ul class="list-group list-group-flush">
                        <li class="list-group-item px-0 d-flex justify-content-between align-items-center">
                            <span class="small fw-medium">Izin Operasional (IJOP)</span>
                        ...
                        <li class="list-group-item px-0 d-flex justify-content-between align-items-center border-bottom-0">
                            <span class="small fw-medium">Data SARPRAS</span>
                        ...
                        <li class="list-group-item px-0 d-flex justify-content-between align-items-center border-bottom-0">
                            <span class="small fw-medium">Rapor Pendidikan</span>
            ...
            <div class="card border-0 shadow-sm rounded-4" style="background-color: #eff4ff;">
                <div class="card-body p-4">
                    <h5 class="font-headline fw-bold mb-3">Modul 4: Rapor Pendidikan</h5>
                    <div class="mb-3">
                        <span class="small text-muted fw-bold d-block mb-1">Dokumen Rapor</span>
                        @if($school->rapor_link) 
                            <a href="{{ $school->rapor_link }}" target="_blank" class="btn btn-sm btn-primary w-100">Buka PDF Rapor Pusat</a>
                        @else 
                            <div class="alert alert-warning py-2 small mb-0">Belum diunggah</div> 
                        @endif
                    </div>
                    <div>
                        <span class="small text-muted fw-bold d-block mb-1">Catatan/Rekomendasi Pengawas</span>
                        <textarea class="form-control form-control-sm border-0 shadow-sm" rows="3" placeholder="Tulis catatan evaluasi di sini..." disabled>{{ $school->catatan_pengawas }}</textarea>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-lg-8">
            <div class="card border-0 shadow-sm rounded-4 mb-4">
                <div class="card-header bg-white border-bottom-0 pt-4 pb-0 px-4 d-flex justify-content-between align-items-center">
                    <h5 class="font-headline fw-bold mb-0">Modul 2: Kontrol KBM</h5>
                    @if(auth()->user()->role === 'admin_sekolah' && auth()->user()->school_id == $school->id)
            ...
            <div class="card border-0 shadow-sm rounded-4">
                <div class="card-header bg-white border-bottom-0 pt-4 pb-0 px-4">
                    <h5 class="font-headline fw-bold mb-0">Modul 3: Laporan Kinerja Wakasek</h5>
                    @if(auth()->user()->role === 'admin_sekolah' && auth()->user()->school_id == $school->id)
                        ...
                        <table class="table align-middle">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>@if($report->kurikulum_link) <a href="{{ $report->kurikulum_link }}" class="text-decoration-none">Cek Berkas</a> @else - @endif</td>
                                    <td>@if($report->kesiswaan_link) <a href="{{ $report->kesiswaan_link }}" class="text-decoration-none">Cek Berkas</a> @else - @endif</td>
                                    <td>@if($report->sarpras_link) <a href="{{ $report->sarpras_link }}" class="text-decoration-none">Cek Berkas</a> @else - @endif</td>
                                    <td>@if($report->humas_link) <a href="{{ $report->humas_link }}" class="text-decoration-none">Cek Berkas</a> @else - @endif</td>
                                    <td class="text-center">
                                                ...
                                                <div class="modal-dialog modal-dialog-centered">
                                                    <div class="modal-content border-0 shadow rounded-4 text-start">
                                                        <div class="modal-header border-bottom-0">
                                                            <h1 class="modal-title fs-6 font-headline fw-bold">Edit Laporan: {{ \Carbon\Carbon::create()->month($report->bulan)->format('F') }}</h1>
                                                        ...
                                                        <form action="{{ route('school.update_monthly_report', $report->id) }}" method="POST">
                                                            ...
                                                            <div class="modal-body py-0">
                                                                <div class="mb-2">
                                                                    ...
                                                                    <input type="url" name="kurikulum_link" class="form-control form-control-sm" value="{{ $report->kurikulum_link }}">
                                                                </div>
                                                                <div class="mb-2">
                                                                    <label class="small fw-bold">Link Kesiswaan</label>
                                                                    <input type="url" name="kesiswaan_link" class="form-control form-control-sm" value="{{ $report->kesiswaan_link }}">
                                                                </div>
                                                                <div class="mb-2">
                                                                    <label class="small fw-bold">Link Sarpras</label>
                                                                    <input type="url" name="sarpras_link" class="form-control form-control-sm" value="{{ $report->sarpras_link }}">
                                                                </div>
                                                                <div class="mb-3">
                                                                    <label class="small fw-bold">Link Humas</label>
                                                                    <input type="url" name="humas_link" class="form-control form-control-sm" value="{{ $report->humas_link }}">
                                                                </div>
                                                            </div>
                                ...
                                @empty
                                <tr><td colspan="5" class="text-center small text-muted py-4">Laporan bulanan belum tersedia.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
                ...
                <div class="modal-body p-4 pt-0">
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <input type="url" name="kurikulum_link" class="form-control" placeholder="https://drive.google.com/...">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label small fw-bold">Link Laporan Kesiswaan</label>
                            <input type="url" name="kesiswaan_link" class="form-control" placeholder="https://drive.google.com/...">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label small fw-bold">Link Laporan Sarpras</label>
                            <input type="url" name="sarpras_link" class="form-control" placeholder="https://drive.google.com/...">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label small fw-bold">Link Laporan Humas</label>
                            <input type="url" name="humas_link" class="form-control" placeholder="https://drive.google.com/...">
                        </div>
                    </div>
                </div>
                <div class="modal-footer bg-light border-top-0 rounded-bottom-4">
...
<div class="modal fade" id="modalDokumenMaster" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content border-0 shadow rounded-4">
            <div class="modal-header border-bottom-0">
                <h1 class="modal-title fs-5 font-headline fw-bold">Update Tautan Dokumen Master</h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
